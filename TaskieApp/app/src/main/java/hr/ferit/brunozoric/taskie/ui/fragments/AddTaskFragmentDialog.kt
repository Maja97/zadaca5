...
class AddTaskFragmentDialog: DialogFragment() {
    ...
    private val currentPriority: String? = PriorityPrefs.getString(PriorityPrefs.KEY_PRIORITY_NAME,"LOW")
    ...
    private fun initUi(){
        context?.let {
            ...
            prioritySelector.setSelection(selectPriority())
        }
    }
    ...
    private fun savePriority(priority: String) {
        PriorityPrefs.store(PriorityPrefs.KEY_PRIORITY_NAME,priority)
    }
    ...
    private fun selectPriority(): Int = when(currentPriority){
        PRIORITY_LOW -> 0
        PRIORITY_MEDIUM -> 1
        PRIORITY_HIGH -> 2
        else -> 0
